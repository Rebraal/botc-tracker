...
function renderRoster() {
  ...
  state.players.forEach((p, idx) => {
    let row = document.createElement('div'); row.className = 'roster-row'; row.dataset.index = idx;
    if (activeMoveIndex === idx) row.classList.add('moving');
    row.onclick = (e) => { if (e.target.tagName === 'SPAN' || e.target.classList.contains('drag-handle')) return; handleRowSelectAction(idx); };
    row.innerHTML = `
    ...
    `;
    let handle = row.querySelector('.drag-handle'); bindMobileRowTouchDragMechanics(handle, row, idx); list.appendChild(row);
  });
...
function bindMobileRowTouchDragMechanics(handle, row, currentIdx) {
  let startY = 0, currentTargetIdx = currentIdx; let rowHeight = 0, siblings = [];
  handle.addEventListener('touchstart', (e) => {
    startY = e.touches.clientY; currentTargetIdx = currentIdx; rowHeight = row.offsetHeight + 4;
    row.style.zIndex = "1000"; row.style.boxShadow = "0 6px 14px rgba(0,0,0,0.6)"; row.style.transition = "none";
  ...
  handle.addEventListener('touchmove', (e) => {
    let currentY = e.touches.clientY; let deltaY = currentY - startY; row.style.transform = `translateY(${deltaY}px)`;
    let calculatedOffsetIndex = Math.round(deltaY / rowHeight); let newTargetIdx = currentIdx + calculatedOffsetIndex;
    ...
    siblings.forEach(sibling => {
      ...
      if (currentIdx < sIdx && sIdx <= currentTargetIdx) sibling.style.transform = `translateY(-${rowHeight}px)`;
  ...
  handle.addEventListener('touchend', () => {
    ...
    if (currentTargetIdx !== currentIdx && !isNaN(currentTargetIdx)) {
      let [movedItem] = state.players.splice(currentIdx, 1); state.players.splice(currentTargetIdx, 0, movedItem); persistData();
    }
    syncUI();
  });
}
...
function handleRowSelectAction(idx) {
  if (activeMoveIndex === null) { activeMoveIndex = idx; updateNotification(`Moving ${state.players[idx].name}. Tap target row to swap seats.`); }
  else { if (activeMoveIndex !== idx) { let temp = state.players[activeMoveIndex]; state.players[activeMoveIndex] = state.players[idx]; state.players[idx] = temp; updateNotification("Seats swapped successfully."); } activeMoveIndex = null; }
  syncUI(); persistData();
}

function addPlayerFromInput() {
  let input = document.getElementById('input-player-name'); let name = input.value.trim(); if (!name) return;
  state.players.push({ name: name, status: 'ALIVE', deadVoteUsed: false, traveler: false }); input.value = ''; input.focus(); syncUI(); persistData();
}
...
function triggerRestartGame() {
  state.votes = []; state.currentDay = 1; state.highestDayReached = 1; state.nominatorToday = []; state.nomineeToday = []; state.toggledDayNVInfo = [];
  state.players.forEach(p => { p.status = 'ALIVE'; p.deadVoteUsed = false; p.role = ""; });
...
function triggerNewGame() {
  state = { players: [], votes: [], currentDay: 1, highestDayReached: 1, setupMode: true, nominationState: 'IDLE', nominatorToday: [], nomineeToday: [], toggledDayNVInfo: [] };
  ...
  if (typeof resetNominationState === 'function') resetNominationState(); setSetupMode(true); updateNotification("New game space completely wiped clean."); syncUI();
}

  ...
  state.players.forEach((p, idx) => {
    ...
    row.innerHTML = `
      <div style="display:flex; align-items:center; gap:8px; width:60%;">
        <span class="drag-handle" style="cursor:grab; padding:2px 6px; background:#444; border-radius:3px; user-select:none;">☰</span>
        <span class="player-label-text" style="pointer-events:none;">${p.name} ${p.traveler ? '(T)' : ''}</span>
      </div>
...
function bindMobileRowTouchDragMechanics(handle, row, currentIdx) {
  let startY = 0; let originalIndex = currentIdx;
  handle.addEventListener('touchstart', (e) => {
    startY = e.touches.clientY; row.style.zIndex = "1000"; row.style.position = "relative";
  }, { passive: true });

  handle.addEventListener('touchmove', (e) => {
    let currentY = e.touches.clientY; let deltaY = currentY - startY; row.style.transform = `translateY(${deltaY}px)`;
    let elementOver = document.elementFromPoint(e.touches.clientX, currentY); if (!elementOver) return;
    ...
    if (targetRow && targetRow !== row) {
      ...
      if (!isNaN(targetIdx) && targetIdx !== originalIndex) {
        let temp = state.players[originalIndex]; state.players[originalIndex] = state.players[targetIdx]; state.players[targetIdx] = temp;
        originalIndex = targetIdx; persistData(); startY = currentY; renderRoster();
      }
    }
  }, { passive: true });
  handle.addEventListener('touchend', () => { row.style.zIndex = ""; row.style.position = ""; row.style.transform = ""; syncUI(); });
}
